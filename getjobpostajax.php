...
if(isset($_POST["action2"]))
{
       ...
       $qry="select * from jobpost where title='$sval'";
	  if(isset($_POST["loc"]) && !empty($_POST["loc"]) )
		  if(isset($_POST["sectitle"]) && !empty($_POST["sectitle"]))
	{
		$qry .= "
	 ...
	 if(isset($_POST["exp"]))
	{
        ...
        foreach($data as $row) {		
			...
			<div class="col-md-12">
            <ul class="accordian">
                <a href="#?id='.'" style="width:100%;"><li id="acco1">'. $row['title'] .' - ' .$row['company_name'] .'</li>
				<i class="fa fa-user" style="margin-left:5%;"></i> <span>'. $row['type'] .'</span>
